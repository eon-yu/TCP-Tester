...
- 패킷 Export/Import로 정의 공유
- JSON 데이터 타입 지원 및 오프셋 자동 조정

## 완료된 작업 요약

- **백엔드**: TCP 서버 CRUD, 패킷 관리 API, 프록시 요청 처리, 상태 모니터링 로직을 구현했습니다. 라우트 파라미터 오류를 수정하고 설정/DB 초기화 테스트를 추가했습니다. Goroutine 기반 패킷 반복 전송과 WebSocket 알림을 도입했습니다. 추가로 패킷 이름/설명과 JSON 데이터를 이력에 저장하고 Export/Import API를 제공합니다.
- **프론트엔드**: 패킷 체인 행 색상 표시, 패킷 에디터 모듈화, 문자열 입력 정리 등 UI 개선을 진행했습니다. API 모듈과 유틸리티 함수에 대한 테스트를 작성했습니다. 요청-응답 페어를 스크롤 목록으로 보여주고 JSON 프레임 뷰어 및 Export/Import 버튼을 추가했습니다.
- **공통**: Go 및 Vitest 기반의 테스트 코드가 함수별로 추가되어 안정성을 높였습니다. WebSocket으로 이력을 즉시 갱신해 요청 시점의 패킷 정보를 보존합니다.
...
| 날짜 | 내용 | 관련 |
| --- | --- | --- |
| 2025-08-19 | 패킷 메타데이터 저장, JSON 타입 지원 및 패킷 Export/Import 기능 추가 | Frontend/Backend |
