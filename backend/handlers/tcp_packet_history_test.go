...
func setupPacketRouter(db *gorm.DB, connManager *services.TCPConnectionManager) *gin.Engine {
	...
	handler := NewTCPPacketHandler(db, connManager, hub, sender)
	tc := r.Group("/api/tcp")
	{
...
func TestSendTCPPacketStoresHistoryWithCRC(t *testing.T) {
	db := setupTestDB()
	connManager := services.NewTCPConnectionManager()
	router := setupPacketRouter(db, connManager)
	...
	server := models.TCPServer{Name: "test", Host: "127.0.0.1", Port: addr.Port}
	db.Create(&server)
	packet := models.TCPPacket{TCPServerID: server.ID, UseCRC: true, Data: models.PacketData{{Offset: 0, Value: 1, Type: models.TypeUint8}}}
	db.Create(&packet)
